...
async function getResources(){const [sets,qualityData]=await Promise.all([Promise.all(DATA_PATHS.map(loadJson)),loadJson('data/quality.json')]);const q=new Map(qualityData.map(x=>[x.id,x]));const seen=new Set();return sets.flat().filter(r=>{if(!r||!r.id||seen.has(r.id))return false;seen.add(r.id);return true}).map(r=>({...r,...(q.get(r.id)||{})}))}
