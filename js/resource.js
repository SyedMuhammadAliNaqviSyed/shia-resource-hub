...
function setMeta(r){
  document.title=`${r.name} — Shia Resource Hub`;
  const desc=r.description||'A curated resource in the Shia Resource Hub directory.';
  let meta=document.querySelector('meta[name="description"]');
  if(!meta){meta=document.createElement('meta');meta.name='description';document.head.appendChild(meta)}
  meta.content=desc.slice(0,160);
  const ensure=(prop,content)=>{let el=document.querySelector(`meta[property="${prop}"]`);if(!el){el=document.createElement('meta');el.setAttribute('property',prop);document.head.appendChild(el)}el.content=content};
  ensure('og:title',`${r.name} — Shia Resource Hub`);
...
async function copyPageLink(btn){
  ...
  try{
    ...
    const prev=btn.textContent;btn.textContent='Copied!';btn.disabled=true;setTimeout(()=>{btn.textContent=prev;btn.disabled=false},1600);
  }catch{
    btn.textContent='Copy failed';setTimeout(()=>{btn.textContent='Copy link'},1600);
  }
}
async function sharePage(r){
  const data={title:r.name,text:r.description||'Found on Shia Resource Hub',url:location.href};
  if(navigator.share){
