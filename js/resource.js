...
const guidance=r=>{
  const text=words(r);
  const purpose=[];
  if(/quran|قرآن|qur'an/.test(text))purpose.push('Quran learning and study');
  if(/hadith|حدیث/.test(text))purpose.push('Hadith learning and reference');
  if(/dua|duas|supplication|دعا|ziyarat|ziyarah/.test(text))purpose.push('Duas and devotional practice');
  if(/prayer|namaz|salah|نماز/.test(text))purpose.push('Prayer learning and practice');
  if(/book|books|library|کتاب/.test(text))purpose.push('Finding books and deeper reading');
  if(/video|lecture|documentary|media/.test(text))purpose.push('Learning through video and media');
  if(/audio|recitation|latmiyat|noha|نوحہ/.test(text))purpose.push('Learning through audio');
  if(/history|karbala|histor/.test(text))purpose.push('Islamic history and context');
  ...
  if(Array.isArray(r.ageRange)&&r.ageRange.length){
    ...
    const aud=(r.audience||[]).map(x=>String(x).toLowerCase());
    const kids=aud.some(a=>/child|kid|parent|famil/.test(a))||/\b(kids|children|child)\b/.test(text);
    const youth=aud.some(a=>/youth|teen|student/.test(a))||/\b(youth|teen|student)\b/.test(text);
    const research=aud.some(a=>/research|scholar/.test(a))||/\b(research|scholar|academic)\b/.test(text);
